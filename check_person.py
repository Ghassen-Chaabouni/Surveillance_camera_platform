...
def check_person():
        ...
        L = os.listdir(r"C:\MAMP\htdocs\unprocessed images" + db_num)
        for ch in L:
            pic = "C:/MAMP/htdocs/unprocessed images" + db_num + "/" + ch
            b = person(pic)
            if (b):
                shutil.move(pic, r"C:\MAMP\htdocs\check faces" + db_num)
            else:
                shutil.move(pic, r"C:\MAMP\htdocs\not a person" + db_num)

        dataset2 = pd.read_csv(r'C:\MAMP\htdocs\dataset2_' + db_num + '.csv')
        dataset2 = dataset2[0:0]
        dataset2.to_csv(r'C:\MAMP\htdocs\dataset2_' + db_num + '.csv', index=False)
        L2 = os.listdir(r"C:\MAMP\htdocs\check faces" + db_num)
        for i in range(len(L2)-1):
            pic1302 = "check faces" + db_num + "/" + L2[i]
            date = L2[i]
        ...
        for key in dict.keys():
            ...
            key = get_date(key[:19])
            new_image.save('C:/MAMP/htdocs/check faces' + db_num + '/' + str(key) + ' (' + str(random.randint(1, 999999999)) + ')' + '.jpg')
