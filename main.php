...
<body style="background-color: #eee;">
            ...
            <div class="collapse navbar-collapse navbar-fixed-top" id="navbarText">
                <ul class="navbar-nav mr-auto">
                    <li class="nav-item active" id="camera_navbar">
                        <?php
                            echo '<a class="nav-link" href="#">Camera</a>';
                        ?>
                    </li>
    ...
    <div id="open_load" align="center" class="content">
        <br/><br/>
        <?php
            echo '<a id="check_persons" href="check_person.php?id='.$id.'"><button class="btn btn-outline-info" name = "check_persons">Check persons</button></a>';
            echo '<a id="check_faces" href="check_faces.php?id='.$id.'"><button class="btn btn-outline-info" name = "check_faces">Check faces</button></a>';
        ?>
        <?php
            echo '<a id="open_camera" href="open_camera.php?id='.$id.'"><button class="btn btn-outline-info" name = "open_camera">Open camera</button></a>';
        ?>
    </div>
	    ...
	    <div class="container">
            <div align="center" id="main">
                ...
                <?php
                    ...
                    $dir1 = "unprocessed images".$id;
                    if (!file_exists($dir1) && !is_dir($dir1)) {
                        mkdir($dir1);
                    }

                    $dir1 = "saved_models".$id;
                    if (!file_exists($dir1) && !is_dir($dir1)) {
                        mkdir($dir1);
                    }

                    $dir1 = "not a person".$id;
                    if (!file_exists($dir1) && !is_dir($dir1)) {
                        mkdir($dir1);
                    }

                    $dir1 = "finish check faces".$id;
                    if (!file_exists($dir1) && !is_dir($dir1)) {
                        mkdir($dir1);
                    }

                    $dir1 = "enter_exit_validation".$id;
                    if (!file_exists($dir1) && !is_dir($dir1)) {
                        mkdir($dir1);
                    }

                    $dir1 = "enter_exit_saved_model".$id;
                    if (!file_exists($dir1) && !is_dir($dir1)) {
                        mkdir($dir1);
                    }
                    ...
                    if (!file_exists($file)) {
                        copy("dataset.csv", "dataset_".$id.".csv");
                    }

                    $file = "dataset2_".$id.".csv";
                    if (!file_exists($file)) {
                        copy("dataset2.csv", "dataset2_".$id.".csv");
                    }
                    ...
                    if($conn->connect_error!=""){
                        ...
                        $sql = "CREATE TABLE `images` (
                            ...
                            name text NOT NULL,
                            imagename VARCHAR(10000) NOT NULL)";
                        mysqli_query($conn, $sql);

                        $sql = "CREATE TABLE `database` (
                            Name varchar(100) NOT NULL,
                            Enter_Exit varchar(100) NOT NULL,
                            Time varchar(100) NOT NULL,
                            Picture varchar(1000) NOT NULL)";
                        mysqli_query($conn, $sql);

                        $sql = "CREATE TABLE `camera` (
                            camera_img varchar(1000) NOT NULL)";
                        mysqli_query($conn, $sql);
                    ...
                    $sql = "SELECT * FROM `database` ORDER BY `Name`";
                    $result = $conn->query($sql);

                    if ($result->num_rows > 0) {
                        // output data of each row
                        while($row = $result->fetch_assoc()) {
	                        echo '<form action="save.php?id='.$id.'" method="post" enctype="multipart/form-data">';
			                ...
			                echo '</form>';
			                echo '<td><a href="delete.php?id='.$id.'&Time='.$row[Time].'&Picture='.$row[Picture].'"><button class="btn btn-outline-danger">Delete</button></a></td>';
			                echo '</tr>';
	                    }
                        echo "</table>";
                    }
